...
# Chemin vers ton projet dbt dans le conteneur
DBT_PATH = "/opt/airflow/dbt_project/senegal_export"
...
with DAG(
    '1_SENEGAL_EXPORT_PIPELINE', # Le nom qui apparaîtra dans Airflow
    default_args=default_args,
    schedule_interval='@hourly', # Automatisation quotidienne
    # schedule_interval=timedelta(hours=12),
    catchup=False
) as dag:

    # Tâche 1 : Charger les nouveaux fichiers CSV (Le Seed)
    task_seed = BashOperator(
        task_id='import_csv_to_postgres',
        bash_command=f'cd {DBT_PATH} && dbt seed --profiles-dir .'
    )

    # Tâche 2 : Transformer les données et calculer le CA (Le Run)
    task_run = BashOperator(
        task_id='transform_and_calculate_CA',
        bash_command=f'cd {DBT_PATH} && dbt run --profiles-dir .'
    )
    # L'ordre d'exécution : d'abord on charge, ensuite on calcule
    task_seed >> task_run
